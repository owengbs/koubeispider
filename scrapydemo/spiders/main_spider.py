...
from scrapydemo.yaolan.yanlan_page_parse import YaolanParser


class DmozSpider(CrawlSpider):

    name = "test"
    allowed_domains = ["www.babytree.com","ask.yaolan.com","http://ask.yaolan.com"]
    start_urls = [
    ...
    rules = (
        ...
        Rule(SgmlLinkExtractor(allow=('http://ask.yaolan.com/period/\d+_\d+.html', 'http://ask.yaolan.com/period/\d+_\d+_\d+.html'),), ),
        Rule(SgmlLinkExtractor(allow=('/question/\d+.html', ),restrict_xpaths=('//div[@class="ask_list_fr"]/div[@class="ask_list"]/ul/li/div[@class="ask_list_title"]/a',)) ,
    ...
    def parse_best_answer(self, response, title):
        best_answers = response.selector.xpath("//div[@id='qa-answer-best']/div[@class='module-content']/div[@class='best-content']")
        ...
        author =bes.xpath("./div[@class='answer-related']/div[@class='qa-contributor']/ul/li/a/span[@itemprop='accountName']/text()").extract()[0]

        return self.build_item(title, create_time, author, response.url, response.url, "", "1", answer_text, "1",self.allowed_domains[0])
    ...
    def parse_answer_section(self, response, title, post_url):
        ...
        for answer in response.selector.xpath("//ul[@class='qa-answer-list']/li[@class='answer-item']"):
            ...
            create_time = answer.xpath("./ul[@class='qa-meta']/li[@class='timestamp']/span[@itemprop='reply_time']/text()")[0].extract()
            content = answer.xpath("./div[@class='answer-text']/text()")[0].extract()

            answer_item = self.build_item(title, create_time, author, response.url, post_url, "", "1", content, "0",self.allowed_domains[0])
